...
#include "esp_event.h"
#include "nvs_flash.h"
#include "bsp/esp-bsp.h"  // For S3-Box-3
#include "elder_ui.h"
#include "mpu6050.h"

#define BUTTON_ORDER_FOOD GPIO_NUM_0
#define BUTTON_HELP GPIO_NUM_39
#define WIFI_SSID "YOUR_WIFI_SSID"  // Replace with your Wi-Fi
#define WIFI_PASS "YOUR_WIFI_PASS"
#define POST_URL "http://192.168.1.100:8000/api/update"  // Replace with your phone's IP/port (e.g., Python server)
...
void send_post(const char* action) {
    esp_http_client_config_t config = { .url = POST_URL, .method = HTTP_METHOD_POST };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    char data[100];
...
void app_main(void) {
    ...
    while (1) {
        ...
        if (gpio_get_level(BUTTON_ORDER_FOOD) == 0) {
            ESP_LOGI(TAG, "Order Food button pressed");
            send_post("order_food");
            show_food_menu();  // Update local UI
            vTaskDelay(pdMS_TO_TICKS(500));  // Debounce
        }
        if (gpio_get_level(BUTTON_HELP) == 0) {
            ESP_LOGI(TAG, "HELP button pressed");
            send_post("help");
            show_confirmation("Help is on the way!");
            vTaskDelay(pdMS_TO_TICKS(500));
        }

        // MPU6050 read (for fall detection)
        float ax, ay, az;
        mpu6050_read_accel(&ax, &ay, &az);
        ESP_LOGI(TAG, "Accel: X=%.2f Y=%.2f Z=%.2f", ax, ay, az);
